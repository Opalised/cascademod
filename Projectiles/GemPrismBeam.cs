...
{
	public class GemPrismBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / GemPrismHoldout.NumBeams;
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<GemPrismBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<GemPrismHoldout>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / GemPrismHoldout.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > GemPrismHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - GemPrismHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * GemPrismHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / GemPrismHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
